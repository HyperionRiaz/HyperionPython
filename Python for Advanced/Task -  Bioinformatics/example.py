...
#Create a program called SickleCellDisease.py . We will simulate the effects of the Single Nucleotide Polymorphism that leads to this genetic diease. 

#Write a function called translate that when given a DNA sequence of arbitary length,
#the program identifies returns the amino acid sequence of the DNA using the amino acid SLC code found in that table.
#EG DNA input: ATTATTATT Output: III

#There are many different amino acids so this may get a bit repetivive. Just do the first five Amino Acids
# (ie I L V F M) and make any other condon be printed as the amino acid 'X' 
#So basicially, you would use an if - elif - elif .... else structure to translate each codon of DNA into the correct Amino Acid
...
#This function must read in the contents of the textfile named 'DNA.txt'
#It must then identify the first occurance of the lowercase letter 'a' in 'DNA.txt'.
#You must then write two new textfiles, one named normalDNA.txt and the other named mutatedDNA.txt.
...
#Now create a new function, txtTranslate, that calls the translate function that you wrote in Part 1, to take in textfile input.
...
#Run this function on the output of Part 2 (The AA of the mutatedDNA.txt file vs the AA of the normalDNA.txt file) and identify the mutated amino acid.
...
#The percentage of aligned (matching) characters should also be outprinted.
