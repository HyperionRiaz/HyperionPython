...
#There will be fewer optional tasks but feel free to go back to the previous tasksif you've forgotten any of the basics!

#When opening this in Notepad++, please make sure you go to View in the Notepad++ toolbar and make sure
#the 'Word Wrap' option is ticked!
...
#		email you a file you need to run to fix this error. YOU CANNOT CONTINUE WITH THE OTHER INSTALLATIONS OR THIS TASK IF YOU GET HTIS ERROR.
#---------------------------------------------------------------------------
#	*STEP 2*
...
#   A box called 'NLTK Downloader' will open. Select 'all-corpora' and then click the 'Download' button on the bottom left.
#	This will download the corpuses that you use in Task 1 of this exercise. If you haven't done this properly, you'll get an error when trying 
# 	to import certain corpuses because they haven't been downloaded to your PC yet. Let us know ASAP if you're having trouble getting the nltk.download() command to work!
#---------------------------------------------------------------------------
...
#	There are *4* seperate installers you should have run - NLTK installer, Numpy installer (either 32bit or 64) PyYaml and matplotlib installer.
...
import nltk #This statement should work fine if you have NLTK, Numpy and PyYaml installed fine

#NLTK is a rich module for Python that has a whole host of features.
#It has a list of 'corpuses' built in to it.
#'Corpus' is the latin word for 'body'. A 'corpus' in NLTK is a body or set of words that form a text.
#For example, NLTK has the speeches that all US presidents gave when they were elected (their inaugural speeches) built into it. 
...
#As you can see there are many built in books and even movie scripts that we can come back 
#to to do some processing of later. 
...
#We can get this by diving the total number of words or TOKENS by the number of unique words ie:
...
#NLTK has a built in function that lets us find the frequency (how often a word occurs in total in the text) of words
#'Distribution' is a fancy word for what we get if we gather up all the individual frequencies of words and store them somewhere. 
#Hence the 'Frequency Distribution' of the words in a text is a sort of table of all the frequencies of each word in a text

from nltk import FreqDist
fd = FreqDist(text1) #We create a frequency distribution of all the words in text1 ie Moby Dick

#Now try:
fd['the'] #Returns the number of times 'the' appears in the text

fd.keys()[0:30] #Prints the first 30 unique words of the text in decreasing order of frequency
fd.items()[0:30] #Does the same as above but also shows the frequency of each word
...
#	The work of a Computer Sciencetist should never just be about programming..
...
#Remember that we said 'duck' could either be a noun (the animal with a beak) or a verb (the action to move your head down) - ie it has two possible part of speech tags depending on the context.

#Before we can begin to tag each word in the sentence, we must first break up the sentence to get each individual word. 
...
print nltk.help.upenn_tagset('VBP') #Queries what POS tag 'VBP' stands for, explains to to you and gives you some examples of words with that tag. 
#Note it uses the UPenn (Uni. of Pennsylvania) set of tags which were created when they first tagged the first corpus of words by hand
...
#	Remember that 'parsing' is when we or a computer try tag words given to us to so we can understand what the sentence actually MEANS

#	Use some Garden Path sentences or think up your own (at least 5). 
#	In your file called garden.py that should be working in for this task, tokenize and pos tag each of the sentences after you have stored them in a list called gardenpathSentences. 
#	See how NLTK has pos tagged these sentences and look up the POS tags you dont understand by using the nltk.help.upenn_tagset command
#	At the bottom of your file, write a comment about one unusual tag you found that NLTK gave one of the words of your sentences - did you expect this? 

# == Optional Challenge ==
# Go to http://en.wikipedia.org/wiki/Garden_path_sentence#Parsing
# Read about the different Parsing strategies. 
# Add a comment at the bottom of your program explaining which parsing strategy you think an AI like Siri (the iPhone 4 talking robot) uses and why? What kind of datastructures might you suggest is used in this process?
# Go to http://www.inf.ed.ac.uk/teaching/courses/inf2a/slides/2011_inf2a_L22_slides.pdf and read up till #slide 16 of this University of Edinburgh slide on Human Parsing. How do you think humans eyes respond #when encountering a Garden Path sentence and what type of parsing (serial or parallel) do you think this is evidence of?
